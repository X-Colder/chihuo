const TOKEN_KEY = 'chihuo.merchant.access-token'
const USER_KEY = 'chihuo.merchant.session-user'
...
function devLogin(force) {
  ...
  return wxLoginCode().then((code) => new Promise((resolve, reject) => {
    wx.request({
      ...
      data: {
        code,
        name: config.DEV_NAME || '开发商家',
        role: config.DEV_ROLE || 'MERCHANT',
        merchant_name: config.DEV_MERCHANT_NAME || '吃货开发演示餐厅'
      },
      header: {
